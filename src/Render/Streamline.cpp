...
{
	interposer = LoadLibraryW(L"Data/SKSE/Plugins/SkyrimUpscaler/Streamline/sl.interposer.dll");
...
bool Streamline::UpdateDLSSG(bool a_enabled, uint a_mode, uint a_numFramesToGenerate, bool a_dynamicMFGEnabled, uint a_dynamicMFGTargetFPS, float2 a_renderSize, float2 a_displaySize, DXGI_FORMAT a_colorFormat, DXGI_FORMAT a_motionVectorFormat, DXGI_FORMAT a_depthFormat, DXGI_FORMAT a_uiFormat)
{
	...
	// With present-override the hud-less is the clean D3D12 DLSS scene and the
	// backbuffer is the composited scene+UI, so UI = backbuffer - hud-less is
	// valid -> let DLSS-G recompose the UI onto generated frames.
...
void Streamline::TagDLSSGResources(ID3D11Texture2D* a_hudlessColor, ID3D11Texture2D* a_motionVectors, ID3D11Texture2D* a_depth, float2 a_renderSize, float2 a_displaySize)
{
	...
	static auto rendererData = Game::GetRendererData();
...
void Streamline::ClearDLSSGResourceTags(ID3D12GraphicsCommandList* a_commandList)
{
	...
	const sl::Extent fullExtent{
		0,
		0,
		static_cast<uint32_t>(graphicsState ? graphicsState->screenWidth : 0),
		static_cast<uint32_t>(graphicsState ? graphicsState->screenHeight : 0)
	};
...
bool Streamline::UpdateConstants(float2 a_jitter)
{
	...
	const bool positionFinite = cameraState &&
		std::isfinite(cameraState->position.x) &&
		std::isfinite(cameraState->position.y) &&
		std::isfinite(cameraState->position.z);
	...
	const float cameraNear = cameraState->cameraNear;
	const float cameraFar = cameraState->cameraFar;
	...
	if (!Util::TryGetCameraBasis(*cameraState, cameraBasis)) {
	...
	const auto currentViewProj = cameraState->viewProjUnjittered;
	const auto previousViewProj = cameraState->prevViewProjUnjittered;
	...
	slConstants.cameraPos = { cameraState->position.x, cameraState->position.y, cameraState->position.z };
