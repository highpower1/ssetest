...
public:
	...
	explicit ConstantBuffer(D3D11_BUFFER_DESC const& a_desc) :
		desc(a_desc)
	{
		auto device = reinterpret_cast<ID3D11Device*>(Game::GetRendererData()->device);
	...
	{
		ID3D11DeviceContext* ctx = reinterpret_cast<ID3D11DeviceContext*>(Game::GetRendererData()->context);
...
public:
	...
	{
		ID3D11Device* device = reinterpret_cast<ID3D11Device*>(Game::GetRendererData()->device);
