...
namespace
{
	bool LoadFidelityFXRuntime()
	{
		...
		constexpr auto pluginDirectory = L"Data\\SKSE\\Plugins\\";
...
void FidelityFX::CopyOpaqueTexture()
{
	...
	// FO4 used kMainTemp (opaque-only main color) as the FSR reactive-mask
	// source. TODO(port): confirm the best Skyrim opaque-color target in-game;
	// kMAIN_COPY is the closest starting point.
	auto source = Game::GetRenderTargetTexture(RE::RENDER_TARGET::kMAIN_COPY);
	if (!source) {
		return;
	}
	...
	bool a_enabled)
{
	...
	prepare.cameraNear = cameraState->cameraNear;
	prepare.cameraFar = cameraState->cameraFar;
	...
	prepare.cameraPosition[0] = cameraState->position.x;
	prepare.cameraPosition[1] = cameraState->position.y;
	prepare.cameraPosition[2] = cameraState->position.z;
	...
	float a_sharpness)
{
	...
	dispatch.cameraNear = Util::CameraFrame::GetSingleton()->cameraNear;
	dispatch.cameraFar = Util::CameraFrame::GetSingleton()->cameraFar;
