...
namespace
{
	...
	Streamline* SelectStreamlineForD3D12Proxy(Streamline* a_streamline, IDXGIAdapter* a_adapter, bool a_wantsD3D12FrameGeneration)
	{
		...
		if (!a_wantsD3D12FrameGeneration) {
			logger::info("[DX12SwapChain] Streamline swapchain interception disabled by the frame-generation safety gate");
			return nullptr;
		}
...
struct hkIDXGIFactoryCreateSwapChain
{
		...
		IDXGISwapChain** ppSwapChain)
	{
		...
		try {
			...
			Streamline* streamlineForProxy = SelectStreamlineForD3D12Proxy(streamline, adapter.get(), Upscaling::kEnableDLSSG);
...
struct hkD3D11CreateDeviceAndSwapChain
{
		...
		ID3D11DeviceContext** ppImmediateContext)
	{
		...
		// Frame-generation experiment: install the factory hook so the game's
		// swapchain creation is intercepted and replaced by the D3D12 proxy.
		...
		// Capture the game D3D11 device/context for the rest of the backend
		// (Game::Renderer, GetRenderTargetTexture, the side-device upscaler) --
		// same responsibility as the minimal hook this replaces.
		{
